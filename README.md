...
-A store is an object that holds the application's state tree. A store holds the whole state tree of your application. this is why its called the single source of truth.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
   -it is a middleware that looks at every action that passes through the system, and if it’s a function, it calls that function. It calls actions that are functions (and returns whatever they return), and otherwise passes the action along to then next middleware, or to Redux itself (which is what next(action) does).

4. What is your favorite state management system you've learned and this sprint? Please explain why!
   -Redux, mainly because it is industry standard. It also helps you write applications that behave consistently, run in different environments (client, server, and native), and are easy to test.
